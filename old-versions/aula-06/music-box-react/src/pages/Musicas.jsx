...
function Musicas() {
  ...
  */
  ...
  useEffect(() => {
    console.log(
      "CHAMADO QUANDO O COMPONENTE É MONTADO, E QUANDO O ESTADO 'numero' É ALTERADO, SOMENTE!"
    );
  ...
  useEffect(() => {
    console.log(
      "CHAMADO QUANDO O COMPONENTE É MONTADO, E QUANDO 'QUALQUER ESTADO' DO COMPONENTE FOR ALTERADO!"
    );
  ...
  // função que será passada como props/parâmetro para o componente ItemMusica.jsx, para que consiga utilizar a função desse componente 
  function deletar(id) {
    api
      .delete(`/${id}`) // chama o método "delete" da instância do axios, passando como parâmetro o ID que a função recebe 
      .then((respostaObtida) => { // trata a resposta da requisição
        console.log(respostaObtida);
        buscarDados();
      })
      .catch((erroOcorrido) => { // trata o erro da requisição
        ...
        <div className="music-boxes">
...

          {
            ...
            */
            ...
            musicas.map((item) => {
              // pega o vetor de músicas, e mapeia música por música, chamando aquele objeto de música do vetor como "item"
              return (
                // retornando 1 componente item música para cada iteração que foi feita no vetor de musicas, utilizando o "item" como parâmetro recebido do elemento iterado
                <ItemMusica
                  key={item.id} // utilizando o id do elemento atual como chave do elemento que está sendo iterado
                  id={item.id} // utilizando o "id" do item atual do vetor como parâmetro "id"
                  ...
                  funcaoDeletar={deletar} // utilizando função "deletar()" como parâmetro "funcaoDeletar", para o componente ItemMusica.jsx conseguir utilizar essa função do Musicas.jsx
                />
              );
            })
          }
